...
import { AlgorithmResult } from '@/types/imageAnalysis';
                        ...
                        <CardContent className="p-5">
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-1 font-medium">{test.description || 'Análisis Forense'}</p>
                                </div>
                                {test.verdict && (
                            ...
                            <div className="space-y-2 mt-4">
                                <div className="flex justify-between text-xs items-end">
                                    ...
                                    <span className="font-bold text-gray-900 text-sm">{test.confidence ? Math.round(test.confidence * 100) : (test.score ? Math.round(test.score * 100) : 0)}%</span>
                                </div>
                                <Progress
                                    value={(test.confidence || test.score || 0) * 100}
