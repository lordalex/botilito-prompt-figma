...
export function CaseFullDetail({ caseId, onBack }: CaseFullDetailProps) {
    const { caseDetail, loading, error } = useCaseDetail(caseId);

    return (
        <div className="flex flex-col h-full bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            {/* Header */}
            <div className="p-6 border-b border-gray-100 bg-gray-50/80 backdrop-blur-sm z-10 shrink-0">
                <div className="flex flex-col gap-4">
                    <Button variant="ghost" size="sm" className="w-fit h-8 px-2 -ml-2" onClick={onBack}>
                    ...
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2 text-xs text-muted-foreground font-mono">
                            <span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded font-semibold">
                                {caseDetail ? ((caseDetail as any).displayId || generateDisplayId(caseDetail as any)) : "..."}
                            </span>
                            <span>•</span>
                            <span>{caseDetail ? new Date(caseDetail.created_at).toLocaleString() : 'Cargando...'}</span>
                        </div>
                        <h2 className="text-2xl md:text-3xl font-bold text-gray-900 leading-tight">
                            {loading ? <Skeleton className="h-10 w-3/4" /> : (caseDetail?.title || "Detalle del Documento")}
                        </h2>
                ...
                <div className="p-6 md:p-8 space-y-8">
                    {loading ? (
                        ...
                        <div className="text-center py-20 text-red-500">
                            ...
                            <p>{error}</p>
                            <Button onClick={onBack} variant="outline" className="mt-4">Regresar</Button>
                        </div>
                    ) : caseDetail ? (
                        <>
                            {/* Summary */}
                                    ...
                                    <section>
                                        ...
                                        <Card className="bg-green-50/30 border-green-100">
                                            <CardContent className="p-5">
                                                ...
                                                {/* Visualization of votes could go here */}
                                            </CardContent>
                                        </Card>
                                    </section>
                                    ...
                                    <section>
                                        ...
                                        <Card className="bg-gray-50">
                                            <CardContent className="p-4">
                                                <div className="max-h-60 overflow-y-auto rounded-md border border-gray-200 bg-white p-4 text-sm text-gray-600">
