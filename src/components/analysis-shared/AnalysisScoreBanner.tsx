...
export function AnalysisScoreBanner({ data, contentType }: AnalysisScoreBannerProps) {
    // Currently only Text analysis (AMI) uses this score banner format
    // But we structure it to be extensible if image/audio need similar banners later

    const { icoScore, diagnosticoAMI } = useTextAnalysisData(data);

    if (contentType !== 'text' || !icoScore) return null;
    ...
    return (
        <div className="w-full mb-8">
            ...
            <div className="bg-gray-900 text-white p-6 md:p-8 rounded-[2rem] shadow-2xl border border-white/5 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden w-full">
                <div
                    className="absolute top-0 right-0 w-64 h-64 bg-primary opacity-10 rounded-full -mr-32 -mt-32"
                    style={{ filter: 'blur(80px)' }}
                />
                <div
                    className="absolute bottom-0 left-0 w-64 h-64 bg-emerald-500 opacity-5 rounded-full -ml-32 -mb-32"
                    style={{ filter: 'blur(80px)' }}
                />
                ...
                <div className="flex-1 text-center md:text-left relative z-10 w-full">
                    <div className="flex flex-col md:items-start items-center">
                        ...
                        <h4 className="text-2xl md:text-3xl font-bold mb-3 tracking-tight text-white">{icoScore.nivel}</h4>
                        <p className="text-sm md:text-base text-slate-400 leading-relaxed max-w-2xl italic">
                            "{diagnosticoAMI || 'Puntuación basada en los criterios de la UNESCO para detectar desinformación.'}"
                        </p>
                    </div>
                </div>
            </div>
